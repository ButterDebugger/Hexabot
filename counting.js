...
export default async (bot) => {
	bot.on("messageCreate", async (message) => {
		const countingChannel = getCountingChannel(bot, message.guild);
		const countingTopic = getCountingTopic(bot, message.guild);
	...
	bot.on("messageUpdate", async (oldMessage, newMessage) => {
		const countingChannel = getCountingChannel(bot, oldMessage.guild);
		if (countingChannel == null) return;
