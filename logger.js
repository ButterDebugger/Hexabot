...
export default async (bot) => {
    bot.on("messageDelete", async (message) => {
		...
		let embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Time Sent', value: time(Math.floor(message.createdTimestamp / 1000), "f") },
                { name: 'Message Content', value: message.content },
                { name: 'Deleted By', value: auditEntry?.executor ? userMention(auditEntry.executor.id) : "Unknown" },
            )
			.setTimestamp()
    ...
    bot.on("messageUpdate", async (oldMessage, newMessage) => {
		...
		let embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Time Edited', value: time(Math.floor(newMessage.editedTimestamp / 1000), "f") },
        ...
        if (auditEntry && isLogged(bot, member.guild.id, "member_kicked")) {
            ...
            let embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: 'Reason', value: auditEntry.reason === null ? "Unspecified" : auditEntry.reason },
                    { name: 'Kicked By', value: auditEntry?.executor ? userMention(auditEntry.executor.id) : "Unknown" },
                )
                .setThumbnail(`https://cdn.discordapp.com/avatars/${member.user.id}/${member.user.avatar}.png`)
                .setTimestamp()
    ...
    bot.on("guildBanAdd", async (ban) => {
        ...
        let embed = new EmbedBuilder()
            ...
            .setThumbnail(`https://cdn.discordapp.com/avatars/${ban.user.id}/${ban.user.avatar}.png`)
            .setTimestamp()
    ...
    bot.on("guildBanRemove", async (ban) => {
        ...
        if (auditEntry) {
            embed.addFields(
                { name: 'Unbanned By', value: userMention(auditEntry.executor.id) },
            )
        }
    ...
    bot.on("inviteCreate", async (invite) => {
        ...
        let embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Expires', value: invite._expiresTimestamp == null ? "Never" : time(Math.floor(invite._expiresTimestamp / 1000), "R") },
                { name: 'Max Uses', value: invite.maxUses == 0 ? "No Limit" : `${invite.maxUses}` },
                { name: 'Created At', value: time(Math.floor(invite.createdTimestamp / 1000), "f") },
            )
            .setTimestamp()
    ...
    bot.on("inviteDelete", async (invite) => {
        ...
        // Check audit logs for more info
        let fetchedLogs = await invite.guild.fetchAuditLogs({
            limit: 6,
        ...
        if (auditEntry) {
            embed.addFields(
                { name: 'Removed By', value: userMention(auditEntry.executor.id) },
            )
        }
